# rgarage_stancer — Installation on vRP

Creative Network and Creative V5 have their own bridges — set `Config.Framework` to
`"creative_network"` or `"creative_v5"`. Everything else on this page is identical.
...
`Config.Framework` is not `"vrp"`, or `@vrp/lib/utils.lua` is still commented. With `Installation.enabled = true` the car also has to have the stancer fitted first.
